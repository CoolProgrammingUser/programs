...
def initialize() :
	...
	for item in speechBubble :
		c.delete(item)
	file = open("SDM highscore.txt", "a+")  # "a+" allows appending and reading (and creates files that don't exist)
	...
	file = open("SDM highscore.txt", "r")  # opens the high score file to read the value (now that the file definitely exists)
	holder = file.read()
...
def keyHandler() :
	...
	binds all used keys
	'''
	for key in ["Escape", "Return", "Right", "d", "D", "6", "Left", "a", "A", "4", "Down", "s", "S", "2", "Up", "w", "W", "8", "space", "5", "n", "N", "y", "Y", "r", "R", "e", "E", "t", "T"]:
...
def restart() :
	...
	c.delete("all")
	for entityClass in [PowerUp, Coin] :
		for entity in entityClass.instances :
			del entity
	speech = []
	repeating = {}
	screen = ""
...
def testGame() :
	'''
	Creates a scene for test purposes.
	Most useful for quickly testing what something looks like.
	'''
	global screen, ground
	c.delete("all")
	screen = "test"
	createBackground(uh(0), uh(220))
	while background[-1][0][0][0] < cw :
...
def jump(height=1) :  # work here too
	...
	if (screen == "outside" or screen == "boss" or screen == "pipe" or screen == "underwater" or screen == "castle") :
		...
		if height == 1 and mario["pos"]["y"] < ch/2-uh(45) :
			# Prevents mario from jumping again while in the air
			return
		if (
...
class Coords :
	...
	def __init__(self, x=None, y=None) :
		if x == None :
			x = rr(round(cw), round(cw*2))  # The randomizing has to be done here or else it'll only be done once.
		if y == None :
			y = ch/2+uh(97)
		self.x = x
		self.y = y
...
class Coin :
	...
	def __init__(self, x=None, y=None) :
		...
		if y != None :
			...
			self.coords.y = rr(round(ch/2-uh(40)), round(ch/2+uh(95)))
		self.shapes = [
			makeOval(self.coords.x, self.coords.y, uh(8), uh(10), fill="#ff0"),
			makeOval(self.coords.x, self.coords.y, uh(4), uh(6), fill="#ff0", outline="#000", width=uh(.5))
		]
		self.touch()
	
	def touch(self, width=8, height=10) :
		'''
		determines if the coin is touching Mario
		width = hit box width
		height = hit box height
		'''
		if (mario["pos"]["x"]+uh(13.5)*size > self.coords.x-uh(width/2)
...
class Boss :
	...
	def __init__(self, ID=None, x=None, y=None) :
		if ID == None :
			self.ID = rc(["Giant Goomba"])
		self.life = 3
		self.coords = Coords()
		# Allows positioning the boss based on what type of boss is about to be drawn
		if x == None :
			self.coords.x = None
		else :
			self.coords.x = x
	...
	def drawBody(self, squish=1) :
		if self.ID == "Bowser" :
			...
			if self.coords.x == None :
				self.coords.x = cw/2
			if self.coords.y == None :
				self.coords.y = ch/2 + uh(78)
			...
			self.shapes = [
				...
				makeOval(self.coords.x-uh(8)*myMultiplier, self.coords.y-uh(4)*squish*myMultiplier, uh(7.5)*myMultiplier, uh(7.5)*squish*myMultiplier, fill="#840"),
				makeOval(self.coords.x+uh(8)*myMultiplier, self.coords.y-uh(4)*squish*myMultiplier, uh(7.5)*myMultiplier, uh(7.5)*squish*myMultiplier, fill="#840"),
				# eyes
...
def createMario(x, y) :
	...
	mario["appearance"].extend([
		...
		makeCircle(myM["x"]+uh(10.75)*size, myM["y"]+uh(19.9)*size, uh(2.12)*size, fill="#840"),
		makeRectangle(myM["x"]-uh(7.5)*size, myM["y"]+uh(20)*size, uh(6.5)*size, uh(4.6)*size, fill="#840"),
		makeCircle(myM["x"]-uh(10.75)*size, myM["y"]+uh(19.9)*size, uh(2.12)*size, fill="#840")])
...
def createEnemy(x, y, variety, squish=1) :
	...
	if variety == "Goomba" :
		...
		enemies[-1].append([
			...
			makeOval(myE[0]-uh(8), myE[1]-uh(4)*squish, uh(7.4), uh(7.4)*squish, fill="#840"),
			makeOval(myE[0]+uh(8), myE[1]-uh(4)*squish, uh(7.4), uh(7.4)*squish, fill="#840"),
			# eyes
		...
		'''
		pass
	elif variety == "Boo" : # You may want to make an enemy class instead of filling this in.
		'''
